...
# Configure logging
log_filename: str = os.path.join(LOGS_DIR, f'sapling_pipe_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
logging.basicConfig(filename=log_filename, level=logging.INFO, 
                    format='%(asctime)s - %(levelname)s - %(message)s')

async def send_to_sapling(text: str) -> str:
    """
    Send text to Sapling AI Detection API asynchronously
    
    Args:
        text (str): Input text to send to Sapling API
    
    Returns:
        str: API response or error message
    """
    # Fetch API key from environment variable
    api_key: Union[str, None] = os.environ.get('SAPLING_API_KEY')
    if not api_key:
        error_msg: str = "SAPLING_API_KEY environment variable not set"
    ...
    try:
        ...
        # Make async API call with Bearer token authorization
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(
                    ...
                    logging.info(f"Sapling API response: {json.dumps(result)}")
...
async def main() -> None:
    ...
    # Send to Sapling API
    result: str = await send_to_sapling(input_text)
